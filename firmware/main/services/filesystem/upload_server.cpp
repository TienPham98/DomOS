...
esp_err_t UploadHandler(httpd_req_t *request)
{
    ...
    httpd_resp_sendstr(request, "{\"ok\":true,\"info\":\"Wallpapers are stored in Webserver Database at http://localhost:8080/api/wallpaper (zero flash wear)\"}");
    return ESP_OK;
}
...
esp_err_t LaunchApiHandler(httpd_req_t *request)
{
    ...
    if (ret > 0 && s_apps != nullptr) {
        ...
        if (std::strstr(buf, "launcher")) app = "launcher";
...
esp_err_t StatusHandler(httpd_req_t *request)
{
    ...
    std::snprintf(response, sizeof(response),
                  ...
                  "\"firmware\":\"0.2.1\",\"online\":true,\"wifi\":{\"ssid\":\"%s\",\"ip\":\"%s\",\"rssi\":%d},"
                  "\"free_heap\":%u,\"storage_used\":40960,\"storage_total\":7340032}",
